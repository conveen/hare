...
impl FromDataPlane<CommittedShortcutList> for hare_control_plane_model::ShortcutList {
    fn convert(shortcut_list: CommittedShortcutList) -> Self {
        hare_control_plane_model::ShortcutList {
            shortcuts: shortcut_list.shortcuts.into_iter().map(hare_control_plane_model::Shortcut::convert).collect(),
        }
    }
}
